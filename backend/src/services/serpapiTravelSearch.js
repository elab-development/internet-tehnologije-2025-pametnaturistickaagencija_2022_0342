/**
 * serpapiTravelSearch.js
 *
 * Šta je ubačeno:
 * - FIX: google_flights departure_id / arrival_id moraju biti IATA (AAA) ili MID (/m/...)
 * - Auto: ako korisnik unese grad/državu (npr. "Budva") -> nađi najbliži aerodrom (IATA) preko:
 *      1) FAST mapa (poznati gradovi)
 *      2) SerpAPI google_locations (ako vrati /m/...)
 *      3) Nominatim geocoding (lat/lon) + airports.json (najbliži IATA)
 * - Cache: geocode + nearest-airport + resolve id (da ne pucaš API stalno)
 *
 * Potrebno:
 * - airports.json (tvoj dataset) u istom folderu (CommonJS require)
 * - Node 18+ (da postoji global fetch) ili instaliraj node-fetch i zameni fetch
 * - SERPAPI_KEY u env
 */
...
// Nominatim zahteva User-Agent; promeni na nešto svoje (kontakt mail optional)
const NOMINATIM_USER_AGENT = 'SerpapiTravelSearch/1.0 (contact: you@example.com)'
const NOMINATIM_ACCEPT_LANGUAGE = 'en'

// FAST map (brzi slučajevi + gradovi bez aerodroma)
const CITY_TO_IATA_FAST = {
  ...
  // gradovi bez aerodroma -> najbliži realni aerodrom
  budva: 'TIV',
  kotor: 'TIV',
  'herceg novi': 'TIV',
  bar: 'TGD',
...
// -------------------- AIRPORT DATA PREP --------------------
// filtriraj dataset samo na aerodrome koji imaju IATA
...
// -------------------- CACHES --------------------
const geocodeCache = new Map() // "budva" -> {lat,lon} | null
const nearestAirportCache = new Map() // "budva" -> { iata, distance_km, name } | null
const resolveIdCache = new Map() // normalized input -> "/m/..." or "AAA" or null
...
// -------------------- OPTIONAL: SERPAPI google_locations -> MID --------------------
...
/**
 * Resolve input (grad/država/aerodrom) -> validan flights id (IATA AAA ili MID /m/...)
 * prioritet:
 * 1) već IATA ili MID
 * 2) FAST mapa
 * 3) SerpAPI google_locations (ako vrati MID)
 * 4) geocode + nearest airport (IATA)
 */
async function resolveFlightId(input, { lang } = {}) {
  ...
  // 3) SerpAPI google_locations -> MID (nekad radi za veće gradove/države)
  const queries = [
    raw,
    `${raw}, ${lang === 'sr' ? 'Srbija' : 'Serbia'}`, // pomaže ponekad kad je input previše kratak
    `${raw} airport`, // pomaže za "Budva airport" (ne uvek)
  ]
  ...
  for (const q of queries) {
    try {
      ...
      // ignoriši i probaj sledeći query
    }
  }

  // 4) fallback: najbliži aerodrom (IATA)
  ...
  const offers = hotels
    .map(h => {
      ...
      return {
        ...
        // ✅ korisno za “details/reviews” kasnije
        provider_meta: {
...
async function searchFlights({ destination, fromCity, from, to, lang, budget }) {
  ...
  try {
    // Resolve oba u validan flights id
...
module.exports = async function serpapiTravelSearch(params) {
  ...
  // console.log({ hotel: hotelOffers[0], flight: flightOffers[0] })
